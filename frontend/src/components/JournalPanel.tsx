...
const JournalPanel: React.FC = () => {
  ...
  const getStatusConfig = (status: string) => {
    switch (status) {
      ...
      default:
        return { label: status, color: 'text-slate-400', bg: 'bg-slate-500/10 border-slate-500/20', icon: AlertCircle };
    }
  };
  ...
  const renderStrategySummary = (strategy: Record<string, any>) => {
    ...
    return <span className="text-xs text-slate-500">策略已生成</span>;
  };
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between p-4 border-b border-slate-700/30">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-medium text-slate-200">{t.journal.title}</h3>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5">
            ...
            <input
              ...
              className="bg-transparent text-xs text-slate-400 outline-none cursor-pointer hover:text-slate-200"
            />
          </div>
          <button
        ...
        <AnimatePresence mode="popLayout">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-xs text-slate-500 mt-3">{t.journal.loading}</p>
            </div>
          ) : error ? (
            ...
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-xs text-slate-500">{t.journal.noEntries}</p>
              <p className="text-[10px] text-slate-600 mt-1">每次Agent运行后会自动生成交易日记</p>
            </div>
          ) : (
            <>
              <div className="text-[10px] text-slate-500 bg-slate-800/30 rounded-lg p-2.5 mb-2">
                        ...
                        <div className="px-4 pb-4 space-y-3">
                          {marketSummary && (
                            <div className="glass-card-inner p-3">
                              <div className="flex items-center gap-1.5 mb-2">
                                ...
                                <span className="text-[11px] font-medium text-slate-400">{t.journal.marketSnapshot}</span>
                              </div>
                              <div className="grid grid-cols-2 gap-2">
                                <div className="flex items-center justify-between px-2 py-1.5 rounded bg-slate-800/30">
                          ...
                          {entry.ai_reasoning && (
                            <div className="glass-card-inner p-3">
                              <div className="flex items-center gap-1.5 mb-2">
                                ...
                                <span className="text-[11px] font-medium text-slate-400">{t.journal.aiReasoning}</span>
                              </div>
                          ...
                          <div className="glass-card-inner p-3">
                            <div className="flex items-center gap-1.5 mb-2">
                              ...
                              <span className="text-[11px] font-medium text-slate-400">{t.journal.strategyAdvice}</span>
                            </div>
                          ...
                          {entry.user_decision && entry.status !== 'created' && (
                            <div className="glass-card-inner p-3">
                              <div className="flex items-center gap-1.5 mb-2">
                                ...
                                <span className="text-[11px] font-medium text-slate-400">{t.journal.userDecision}</span>
                              </div>
                              ...
                              {entry.user_decision.notes && (
                                <p className="text-xs text-slate-400 mt-1">{entry.user_decision.notes}</p>
                              )}
                            </div>
                          )}
                          ...
                          {entry.reflection && (
                            <div className="glass-card-inner p-3 border-l-2 border-l-emerald-500/50">
                              <div className="flex items-center gap-1.5 mb-2">
                                ...
                                <span className="text-[11px] font-medium text-slate-400">{t.journal.reflection}</span>
                              </div>
                              <p className="text-xs text-slate-300 leading-relaxed whitespace-pre-wrap">
                                {entry.reflection}
                              </p>
                              {entry.iteration_notes && entry.iteration_notes.length > 0 && (
                                <div className="mt-2 pt-2 border-t border-slate-700/20">
                                  <p className="text-[10px] text-slate-500 mb-1">迭代改进:</p>
